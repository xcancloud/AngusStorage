...
public interface SpaceRepo extends BaseRepository<Space, Long>, NameJoinRepository<Space, Long> {
  ...
  @Query(value = "SELECT id FROM storage_object_space WHERE bucket_name = ?1", nativeQuery = true)
  Set<Long> findIdByBucketName(String name);

  @Query(value = "SELECT s.* FROM storage_object_space s WHERE s.id IN (SELECT so.space_id FROM storage_space_object so WHERE so.space_id in (?1))  limit 1", nativeQuery = true)
  Space findNotEmptyBySpaceIdInLimit1(Collection<Long> spaceIds);

  @Query(value = "SELECT s.* FROM storage_object_space s WHERE s.biz_key = ?1 limit 1", nativeQuery = true)
  Space findByBizKeyLimit1(String bizKey);

  @Query(value = "SELECT s.* FROM storage_object_space s WHERE s.tenant_id = ?1 AND s.biz_key = ?2 limit 1", nativeQuery = true)
  Space findByTenantIdAndBizKeyLimit1(Long optTenantId, String bizKey);

  @Query(value = "SELECT id FROM storage_object_space WHERE id IN ?1 AND auth = ?2 ", nativeQuery = true)
  ...
  @Modifying
  @Query(value = "DELETE FROM storage_object_space WHERE id in ?1", nativeQuery = true)
